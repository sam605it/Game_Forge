...
export const buildPlatformerSpec = (prompt: string, params: DesignParams, seed: number): { spec: GameSpecV1; templateId: string } => {
  const template = TEMPLATE_MAP.get("platformer") ?? TEMPLATE_MAP.get("runner");
  if (!template) {
