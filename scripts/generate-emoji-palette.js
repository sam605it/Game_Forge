...
const makeId = (emoji) => Array.from(emoji).map((char) => char.codePointAt(0).toString(16)).join("-");

const buildFallback = () => {
  const entries = FALLBACK_SEED.map((entry) => ({
    ...entry,
  ...
  const emojiModifierBases = new Set();
  ...
  const pushEntries = (list, extraKeywords = [], extraGroups = []) => {
    list.forEach((emoji) => {
